package org.jmicro.main.monitor.server.impl;
...
public class MonitorServerImpl implements IMonitorServer {
	...
	public void ready() {
		...
		ServiceItem si = sl.createSrvItem(IMonitorAdapter.class, Config.getInstanceName()+"."+MonitorServerStatusAdapter.class.getName(), "0.0.1", null);
		...
		if(!subsribers.isEmpty()) {
			for(IMonitorDataSubscriber m : this.subsribers){
				subscriberChange(m,IServiceListener.ADD);
			}
			//doSubmitCacheItems();
		}
			...
			try {
				...
				while(ite.hasNext()) {
					...
					if(curTime - ri.lastSendTime >= sendInterval) {
						if(monitoralbe) {
							sc.add(MonitorConstant.Ms_CheckerSubmitItemCnt, ri.cacheItems.size());
						}
						ri.isWorking = true;
				...
				if(monitoralbe) {
					sc.add(MonitorConstant.Ms_CheckerExpCnt, 1);
				}
			}
		}
	}
	...
	private boolean addOneMonitor(IMonitorDataSubscriber m) {
		try {
			...
			Set<Short> types = this.monitorManager.intrest(skey);
	...
	private class Worker implements Runnable{
		...
		public void run() {
			try {
				...
				logger.error("",e);
				if(monitoralbe) {
					sc.add(MonitorConstant.Ms_TaskFailItemCnt, ri.sendItems.size());
				}
			} finally {
				ri.isWorking = false;
	...
	public class MonitorServerStatusAdapter implements IMonitorAdapter{
		...
		public final Short[] TYPES  = {
				MonitorConstant.Ms_ReceiveItemCnt,MonitorConstant.Ms_TaskSuccessItemCnt,MonitorConstant.Ms_CheckLoopCnt,
				MonitorConstant.Ms_FailItemCount,MonitorConstant.Ms_CheckerSubmitItemCnt,
				MonitorConstant.Ms_Fail2BorrowBasket,MonitorConstant.Ms_FailReturnWriteBasket,
				MonitorConstant.Ms_CheckerExpCnt,MonitorConstant.Ms_TaskFailItemCnt
		};

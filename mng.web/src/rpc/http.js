...
export default  {
    ...
    ,uploadBase64Picture: function (data, ptype, cb,pos) {
        ...
        this.post(url, params, (data1, status, xhr)=> {
            console.log(data1);
            if (status !== 'success') {
                cb(status, data1);
            } else {
    ...
    ,postHelper(url,params,cb){
        this.post(url,params,(data,status,xhr)=>{
            if(data.success) {
                cb(null,data);
            } else {
                cb(data,data);
    ...
    ,getHelper(url,params,cb,fullpath){
        ...
        this.get(url,params,(data,status,xhr)=>{
            cb(null,data);
        },function(err){
            cb(err,null);
        },fullpath);
    ...
    getWebResourceHttpPath : function(subPath) {
        var wp = 'http://' + location.host + utils.getWebContextPath()
            + subPath;
        return wp;
    }
}
...
export default{
    ...
    encrypt: function (msg){
        ...
        if(!cfg.pwd || new Date().getTime() - cfg.lastUpdatePwdTime > 1000*60*5 ) {
            ...
            cfg.pwd = this.genStrPwd(16);
            //告诉服务端，有AES密码更新
            msg.setSec(true);
            //对AES密码做RSA加密
            msg.setSecData(this.encryptRas(cfg.pwd));
        }else if(new Date().getTime() - cfg.lastUpdatePwdTime < 2*1000) {
            //前5秒内的包都带上密钥，避免消息发送乱序情况下服务器解密错误
            msg.setSec(true);
            //msg.sec = this.encryptRas(cfg.pwd);
            msg.setSecData(this.encryptRas(cfg.pwd));
            msg.setSecretVersion(cfg.secretVersion);
        }
        ...
        var encrypted = CryptoJS.AES.encrypt(b64Str, CryptoJS.enc.Utf8.parse(cfg.pwd),opts);
    ...
    encryptRas:function(strContent) {
        ...
        let rst = cfg.rsae.encrypt(strContent);

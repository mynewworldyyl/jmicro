<template>
    <div class="JPermissionApproveList">

        <table v-if="isLogin && authList && authList.length > 0" class="configItemTalbe" width="99%">
          <thead><tr>
			  <td>{{'Service'|i18n}}</td><td>{{'Method'|i18n}}</td>
			  <td>{{'Namespace'|i18n}}</td><td style="width:70px">{{'Version'|i18n}}</td>
			  <td>{{'ClientId'|i18n}}</td><td>{{'Status'|i18n}}</td>
              <td>{{'actName'|i18n}}</td> 
			  <td>{{'ForId'|i18n}}</td><td>{{'Type'|i18n}}</td>
			  <td>{{'UpdatedTime'|i18n}}</td>
              <td>{{'Operation'|i18n}}</td></tr>
          </thead>
          <tr v-for="a in authList" :key="a.ma.id">
			  <td :title="a.serviceName">{{sn(a.serviceName)}}</td><td>{{a.method}}</td><td>{{a.namespace}}</td><td>{{a.version}}</td>
			  <td>{{a.ma.clientId}}</td><td>{{authStatus[a.ma.status]}}</td>
			  <td>{{a.ma.actName || '*'}}</td>
			  <td>{{a.ma.forId}}</td><td>{{a.ma.type==1 ? '角色':'账号'}}</td>
			  <td>{{a.ma.updatedTime|formatDate(1)}}</td>
			 <!-- <td v-html="a.ma.remark"></td> -->
              <td><a v-if="a.ma.status == 3"   @click="authAct(a.ma,4)"> {{'Revoke' | i18n}} </a>
                  <a v-if="a.ma.status == 1 || a.ma.status == 4"   @click="authAct(a.ma,3)"> {{'Approve' | i18n}} </a>
                  <a v-if="a.ma.status == 1"   @click="authAct(a.ma,2)"> {{'Reject' | i18n}} </a>
              </td>
          </tr>
        </table>
        ...
        <Drawer v-if="isLogin"   v-model="query.drawerStatus" :closable="false" placement="left"
            ...
            <table>
                <tr>
                    ...
                    <td>
                        <Input  v-model="qry.ps.serviceName"/>
                    </td>
                    <td>{{"Namespace"|i18n}}</td>
                    <td>
                        <Input  v-model="qry.ps.namespace"/>
                    </td>
                </tr>
                <tr>
                    <td>{{"Version"|i18n}}</td>
                    <td>
                        <Input  v-model="qry.ps.version"/>
                    </td>
                    <td>{{"Status"|i18n}}</td>
                    <td>
                ...
                <tr>
                    <td>{{"Method"|i18n}}</td>
                    <td>
                        <Input  v-model="qry.ps.method"/>
                    </td>
                    <td>{{"Hash"|i18n}}</td>
                    <td>
                        <Input v-model="qry.ps.haCode"/>
                    </td>
                </tr>
    ...
    export default {
        ...
        methods: {
            ...
            refresh(){
                ...
                this.$jr.rpc.callRpcWithParams(sn, ns, v, 'queryMethodList', [this.qry])
                    .then((resp) => {
                        if (resp.code != 0) {
            ...
            descAuthOk() {
                ...
                this.$jr.rpc.callRpcWithParams(sn, ns, v, 'updateAuthStatus',
                    [this.auth.id,this.status,this.auth.desc])
                    .then((resp) => {
                        if (resp.code != 0) {
                            this.errMsg = resp.msg;
                            return;

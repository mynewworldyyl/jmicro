		...
		if (__val5 == null) {
			...
			if (size > 0) { // if block1
				...
				if (java.lang.String.class != null && (java.lang.reflect.Modifier
					...
					boolean sameKeyElt = cn.jmicro.agent.SerializeProxyFactory.sameCollectionTypeEles(__val5.keySet());// 是否是同种类型的对象
					boolean isKeyFinal = cn.jmicro.agent.SerializeProxyFactory
							.seriaFinalClass(__val5.keySet().iterator().next().getClass());
					boolean hasNullKeyElt = cn.jmicro.agent.SerializeProxyFactory.collHasNullElement(__val5.keySet());// 是否有空元素的KEY
				...
				if (java.lang.Object.class != null && (java.lang.reflect.Modifier
					...
					boolean sameValElt = cn.jmicro.agent.SerializeProxyFactory.sameCollectionTypeEles(__val5.values());
					boolean isValFinal = cn.jmicro.agent.SerializeProxyFactory
							.seriaFinalClass(__val5.values().toArray());
					boolean hasNullValElt = cn.jmicro.agent.SerializeProxyFactory.collHasNullElement(__val5.values());// 是否有空元素的值
				...
				while (ite.hasNext()) { // loop block5
					...
					cn.jmicro.agent.SerializeProxyFactory.encodeListElement(__buffer, key);
					if (writeValEvery) {
						if (val == null) {
							__buffer.writeByte(cn.jmicro.api.codec.Decoder.PREFIX_TYPE_NULL);
							continue;
					...
					cn.jmicro.agent.SerializeProxyFactory.encodeListElement(__buffer, val);
				} // end for loop block5
				out.write(flagIndex5, flag5);
			} // end if block1
		} // end else block0

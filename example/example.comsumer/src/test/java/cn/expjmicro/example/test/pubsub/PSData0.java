...
public final class PSData0 implements Serializable, ISerializeObject {
	...
	public void encode(java.io.DataOutput __buffer) throws java.io.IOException {
		...
		if (__val5 == null) {
			...
			if (size > 0) { // if block1
				...
				if (java.lang.String.class != null && (java.lang.reflect.Modifier
					...
					flag5 |= cn.jmicro.common.Constants.GENERICTYPEFINAL; // 能从泛型中能获取到足够的列表元素类型信息
				} else { // block2
					boolean sameKeyElt = cn.jmicro.agent.AddSerializedToObject.sameCollectionTypeEles(__val5.keySet());// 是否是同种类型的对象
					boolean isKeyFinal = cn.jmicro.agent.AddSerializedToObject
							.seriaFinalClass(__val5.keySet().iterator().next().getClass());
					boolean hasNullKeyElt = cn.jmicro.agent.AddSerializedToObject.collHasNullElement(__val5.keySet());// 是否有空元素的KEY
				...
				if (java.lang.Object.class != null && (java.lang.reflect.Modifier
					...
					flag5 |= cn.jmicro.common.Constants.EXT0;
				} else { // block2
					boolean sameValElt = cn.jmicro.agent.AddSerializedToObject.sameCollectionTypeEles(__val5.values());
					boolean isValFinal = cn.jmicro.agent.AddSerializedToObject
							.seriaFinalClass(__val5.values().toArray());
					boolean hasNullValElt = cn.jmicro.agent.AddSerializedToObject.collHasNullElement(__val5.values());// 是否有空元素的值
				...
				while (ite.hasNext()) { // loop block5
					...
					if (writeKeyEvery) {
						...
						if (cc5 == null) {
							cn.jmicro.agent.AddSerializedToObject.errorToSerializeObjectCode(key.getClass().getName());
					...
					cn.jmicro.agent.AddSerializedToObject.encodeListElement(__buffer, key);
					if (writeValEvery) {
						if (val == null) {
							__buffer.writeByte(cn.jmicro.api.codec.Decoder.PREFIX_TYPE_NULL);
							continue;
						...
						if (cc5 == null) {
							cn.jmicro.agent.AddSerializedToObject.errorToSerializeObjectCode(val.getClass().getName());
							__buffer.writeByte(cn.jmicro.api.codec.Decoder.PREFIX_TYPE_PROXY);
					...
					cn.jmicro.agent.AddSerializedToObject.encodeListElement(__buffer, val);
				} // end for loop block5
				out.write(flagIndex5, flag5);
			} // end if block1
		} // end else block0
		...
		__buffer.writeByte(0); // forward one byte
		if (__val9 == null) {
			flag9 |= cn.jmicro.common.Constants.NULL_VAL;
		...
		cn.jmicro.api.registry.UniqueServiceMethodKey __val10 = __obj.callback;
		byte flag10 = 0;
		...
		if (__val10 == null) {
			...
			((cn.jmicro.api.codec.ISerializeObject) __o10).encode(__buffer);
			out.write(flagIndex10, flag10);
		} // end else block0
	...
	public void decode(java.io.DataInput __buffer) throws java.io.IOException {
		...
		if (0 != (cn.jmicro.common.Constants.NULL_VAL & flagName5)) {
			...
			if (__obj.context == null) { // block0
				Class cls = null;
				if (clsName != null) {
					cls = cn.jmicro.agent.AddSerializedToObject.loadClazz(clsName);
				} else {
					cls = cn.jmicro.api.codec.TypeCoderFactory.getIns().getClassByCode(new Short(c));
				}
				__val5 = (java.util.Map) cn.jmicro.agent.AddSerializedToObject.newInstance(cls);
			...
			if (size > 0) { // block2
				...
				c = 0;
				if (0 == (cn.jmicro.common.Constants.GENERICTYPEFINAL & flagName5)) { // blockgenic 从头部中读取类型信息
					if (0 != (cn.jmicro.common.Constants.HEADER_ELETMENT & flagName5)) {
						readKeyEvery = false;
						if (0 != (cn.jmicro.common.Constants.ELEMENT_TYPE_CODE & flagName5)) {
							...
							keyEleCls = cn.jmicro.agent.AddSerializedToObject.loadClazz(keyClsName);
						}
					}
				} // blockgenic
				else {
				...
				c = 0;
				if (0 == (cn.jmicro.common.Constants.EXT0 & flagName5)) { // blockgenic 不能从泛型参数获取类型信息
					if (0 != (cn.jmicro.common.Constants.EXT1 & flagName5)) {
						readValEvery = false;
						if (0 != (cn.jmicro.common.Constants.SIZE_NOT_ZERO & flagName5)) {
							valClsName = __buffer.readUTF();
							valEleCls = cn.jmicro.agent.AddSerializedToObject.loadClazz(valClsName);
						} else {
							c = __buffer.readShort();
				...
				while (cnt < size) { // block5
					...
					if (readKeyEvery) { // block6
						...
						if (prefixCode == cn.jmicro.api.codec.Decoder.PREFIX_TYPE_SHORT) {
							...
							keyEleCls = AddSerializedToObject.loadClazz(cn);
						}
					} // block6
					Object key = cn.jmicro.agent.AddSerializedToObject.decodeListElement(__buffer, keyEleCls);
					if (readValEvery) { // block6
						...
						if (prefixCode == cn.jmicro.api.codec.Decoder.PREFIX_TYPE_SHORT) {
							...
							java.lang.String cn = __buffer.readUTF();
							valEleCls = AddSerializedToObject.loadClazz(cn);
						}
					} // block6
					Object val = cn.jmicro.agent.AddSerializedToObject.decodeListElement(__buffer, valEleCls);
					if (key != null) { // block7
						__val5.put(key, val);
					} // block7
				} // block5
		...
		cn.jmicro.api.registry.UniqueServiceMethodKey __val10;
		byte flagName10 = __buffer.readByte();
		__val10 = null;
		if (0 != (cn.jmicro.common.Constants.NULL_VAL & flagName10)) {
			__val10 = null;
		} else { // block0
			__val10 = new cn.jmicro.api.registry.UniqueServiceMethodKey();
			((cn.jmicro.api.codec.ISerializeObject) (Object) __val10).decode(__buffer);
		} // block0
